...
function myIP(){
  document.getElementById('myipreload').style.display='none';
  document.getElementById('myipreloading').style.display='inline';
  const options = {method: 'GET'};
  fetch('https://ipwho.is/', options)
    .then(response => response.json())
    .then(response => document.getElementById('myipdisp').innerHTML=response.ip)
    .then(document.getElementById('myipreload').style.display='inline')
    .then(document.getElementById('myipreloading').style.display='none')
    .catch(err => console.error(err));
}